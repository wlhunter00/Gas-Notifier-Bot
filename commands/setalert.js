  ...
  async execute(interaction, db) {
    ...
    if (gasLevel < 0 || gasLevel > 10000) {
      await interaction.reply({
        content: `${gasLevel} is out of reasonable range for GWEI!`,
        ephemeral: true,
      });
      return;
    }
    ...
    if (gasObj) {
      ...
      const userInserted = gasObj.userLists.find(
        (user) => user.userID === interaction.user.id
      );
      if (userInserted) {
        console.log(`Alert already set, user ${userInserted.userID}`);
        await interaction.reply({
          content: `You already have an alert set at ${gasLevel} GWEI!`,
          ephemeral: true,
        });
        return;
      }
      // If not on the list
        ...
        const extendedList = [...gasObj.userLists, newUser];
    ...
    // If object not already created
      ...
      console.log(
        `Created new tracking alert for ${gasLevel} GWEI, for user ${interaction.user.username}, with ID:${result.insertedId}`
      );
    }
    // Send confirmation message
    await interaction.reply({
      content: `Set alert at ${gasLevel} GWEI!`,
      ephemeral: true,
    });
  },
};
