...
setInterval(async function () {
  ...
  if (clientLogin) {
    try {
      ...
      client.user.setActivity(`⚡${gasPrice} GWEI /setalert`, {
        type: "WATCHING",
      });

      // Retrieve all gas objects from api lower than value with a non-empty userlist
      const query = {
        gasLevel: { $gt: gasPrice + 1 },
        userLists: { $exists: true, $type: "array", $ne: [] },
      };
        ...
        for await (const gasObject of gasObjects) {
          ...
          for await (const user of userList) {
            ...
            if (!notifiedUserIDs.includes(user.userID)) {
              ...
              sendUser.send(
                `Hey <@${user.userID}>, gas is at ${gasPrice} GWEI! (You wanted to be notified once gas was lower than ${gasObject.gasLevel} GWEI)`
              );
